...
class Main extends PluginBase {
    ...
    switch(strtolower($command->getName())) {
      case "vote":
        ...
        if(!Utils::hasPermission($sender, "votereward.command.vote")) {
          $sender->sendMessage("§3You do not have permission to use this command.");
          break;
        }
        if(in_array(strtolower($sender->getName()), $this->queue)) {
          $sender->sendMessage("§3Vote §cSlow down! We're already checking lists for you.");
          break;
        }
  ...
  public function rewardPlayer($player, $multiplier) {
    ...
    $player->sendMessage("§3vote §dYou voted on §b$multiplier §3server list" . ($multiplier == 1 ? "" : "s") . "!");
  }
